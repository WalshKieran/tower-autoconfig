...
from tower_autoconfig.utils import create_ssh_key
...
class TowerAutoconfig:
    ...
    async def prepare(self, compute_name: str, credentials_name: str, new_pipelines: List[str], pipeline_suffix: str, shouldForce: bool):
        ...
        if not skipPipe:
            ...
            new_pipeline_set = set()
            for p in new_pipelines:
                if p in remote_pipelines_dict:
                    new_pipeline_set.add(p)
                else:
            ...
            pipelines_add = new_pipeline_set if shouldForce else (new_pipeline_set - pipeline_name_to_id.keys())
            pipelines_remove = pipeline_name_to_id.keys() - new_pipeline_set

            old_label_set = {label['name'] for p in pipelines if p['fixed_name'] in pipelines_remove for label in p['labels']}
            new_label_set = {topic for p in new_pipeline_set for topic in remote_pipelines_dict[p]['topics']}
            labels_remove = old_label_set - new_label_set
            labels_add = new_label_set - label_name_to_id.keys()

            pipeline_ret = (
                list(pipelines_add), list(pipelines_remove), pipeline_name_to_id, remote_pipelines_dict,
                list(labels_add), list(labels_remove), label_name_to_id
            )
        
        return (user, compute_id, compute_primary_id, credentials_id, *pipeline_ret)

    async def setup_ssh(self, compute_name: str, compute_description: str, compute_platform: str, compute_host: str, compute_user: str, workdir: str,
        ...
        if not compute_id or shouldForce:
            if not credentials_id or shouldForce:
                ssh_key = create_ssh_key(f'mykey:{self.server}', ssh_restrictions)
                credentials_id = await self.api.add_credentials_ssh(credentials_name, credentials_description, credentials_id, ssh_key)

            try:
                compute_id = await self.api.add_compute(compute_name, compute_description, compute_platform, compute_host, compute_user, credentials_id, workdir, compute_id)
            except TowerApiError as e:
                raise TowerAutoconfigError("Failed create compute environment - this usually means your machine isn't open to the internet (try agent method)") from e

        return compute_id, credentials_id

    async def setup_agent(self, compute_name: str, compute_description: str, compute_platform: str, compute_host: str, compute_user: str, workdir: str,
    ...
    async def setup_pipelines(self, compute_id, pipelines_add, pipeline_name_to_id, label_name_to_id, remote_pipelines_dict, pipeline_suffix, config_text, prerun_text, workdir, profiles):
        ...
        for p in pipelines_add:
            ...
            tasks.append(self.api.add_pipeline(p.split('/', 1)[-1] + pipeline_suffix, description, icon, pipeline_url, compute_id, label_ids, config_text, prerun_text, workdir, profiles, existing_id))
